...
void YoloObjectDetector::init()
{
  ...
  // Traffic Light
  std::string trafficLightImageTopicName;
  int trafficLightImageQueueSize;
  bool trafficLightImageLatch;
  ...
  nodeHandle_.param("publishers/traffic_light_image/topic", trafficLightImageTopicName,
                    std::string("traffic_light_image"));
...
void YoloObjectDetector::publishTrafficLightImage(const cv::Mat& traffic_light_image)
{
  cv_bridge::CvImage cvImage;
  cvImage.header.stamp = ros::Time::now();
  cvImage.header.frame_id = "traffic_light_image";
  cvImage.encoding = sensor_msgs::image_encodings::BGR8;
  cvImage.image = traffic_light_image;
  trafficLightImagePublisher.publish(*cvImage.toImageMsg());
  ROS_DEBUG("Traffic light image has been published.");
}
    ...
    for (int i = 0; i < numClasses_; i++) {
      if (rosBoxCounter_[i] > 0) {
        ...
        for (int j = 0; j < rosBoxCounter_[i]; j++) {
          ...
          // Only Detect Traffic Light Image
          if (boundingBox.Class == "traffic light") {
            cv::Mat image;
            cvImage(cv::Rect(boundingBox.xmin, boundingBox.ymin, 
              boundingBox.xmax - boundingBox.xmin, boundingBox.ymax - boundingBox.ymin)).copyTo(image);

            publishTrafficLightImage(image);
          }
